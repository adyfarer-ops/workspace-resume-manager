...
**重要规则：**
1. `location = /ady/api/resume/pdf` 必须使用 `=` 精确匹配
2. 必须放在 `location /ady/` 之前
3. proxy_pass 后面不要加 `/`，保持原始路径

### PDF API 关键逻辑
```javascript
// 1. 头像处理 - 转为 base64 避免网络加载问题
let avatarBase64 = '';
try {
...
app.get('/ady/api/resume/pdf', async (req, res) => { ... });  // Nginx 代理路径
```

### 部署检查清单
- [ ] PDF API 服务运行在端口 3002
- [ ] Nginx 配置包含 PDF API location
- [ ] 头像文件 `/var/www/ady/avatar.png` 存在
- [ ] 数据库 `profiles.avatar` 字段为 `/ady/avatar.png`
- [ ] 静态文件已复制到 `/var/www/yfarer.cn/ady/`

### 故障排查
| 问题 | 原因 | 解决 |
|-----|------|------|
| PDF 下载 404 | Nginx 未代理到 3002 | 检查 location 配置和 PDF API 是否运行 |
| PDF 无头像 | Puppeteer 无法加载网络图片 | 使用 base64 嵌入头像 |
| 网站 404 | 静态文件未部署 | 复制 dist 到 /var/www/ady/ 和 /var/www/yfarer.cn/ady/ |
| 样式不生效 | 浏览器缓存 | 强制刷新或无痕模式 |

### 常用命令
```bash
# 重启 PDF API
pkill -f pdf-server; cd /root/.openclaw/workspace-resume-manager/api && nohup node pdf-server.js > /tmp/pdf-server.log 2>&1 &

# 检查 PDF API 状态
curl -s http://localhost:3002/api/health

# 测试 PDF 生成
curl -s http://localhost:3002/ady/api/resume/pdf -o /tmp/test.pdf

# 重启 Nginx
/usr/sbin/nginx -t && /usr/sbin/nginx -s reload

# 构建并部署
cd /root/.openclaw/workspace-resume-manager/project && npm run build
cp -r dist/* /var/www/ady/
cp -r /var/www/ady/* /var/www/yfarer.cn/ady/
```

### Favicon
- **文件位置**: `/var/www/ady/favicon.png`
- **风格**: 动漫风格头像
- **特征**: 黑色短发、黑色上衣
- **背景**: 蓝色渐变
- **边框**: 圆角
- **描述**: 圆角风格的动漫头像，蓝色背景，黑色短发男生形象

### Avatar (头像)
- **文件位置**: `/var/www/ady/avatar.png`
- **用途**: 网站和 PDF 简历中显示的个人头像
- **风格**: 与 favicon 一致
