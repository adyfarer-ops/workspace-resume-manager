...
const PROFILE_DATA = {
  ...
  skills: [
    {
      title: "AI 智能体开发",
      skills: ["了解n8n和dify", "熟练使用Coze(扣子)", "掌握Workflow编排", "插件调用及JS代码节点编写", "Prompt Engineering擅长风格迁移", "角色扮演与任务拆解"]
    },
    {
      title: "Web开发",
      skills: ["小程序", "VUE", "HTML", "CSS", "JavaScript"]
    },
    {
      title: "AI工具使用",
      skills: ["Kimi.ai", "GPT", "Gemini", "Sora2", "即梦", "豆包"]
    }
  ],
  workExperience: [
    {
  ...
  projects: [
      ...
      subProjects: [
        {
          title: "个人小程序【最后亿次】",
          subtitle: "已上架",
          description: "AI工具：Kiro, Antigravity, DeepSeek",
    ...
    },
    {
      ...
      subProjects: [
        {
          title: "茶文化风格化文案生成智能体",
          description: "解决文案同质化问题，降低人工模仿头部博主语气的成本。"
        },
        {
          title: "\"家有大猫\"沉浸式角色扮演",
          description: "深度定义角色（如林虎）口癖与性格，设计\"好感度\"系统，实现高拟真对话。"
        }
      ],
      tags: ["Coze", "AI智能体", "工作流"]
    }
...
<body>
  <div class="resume">
    <div class="sidebar">
      <div class="avatar-wrap"><div class="avatar"><img src="${avatarDataUrl}" alt="头像"></div></div>
      ...
      <div class="s-section">
        ...
        <div class="s-row"><strong>电话：</strong>${atob(data.contact.phone)}</div>
      ...
      <div class="s-section">
        ...
        <div class="skill-item">
          ...
          <div class="skill-desc">${s.skills.map(skill => typeof skill === 'string' ? skill : skill.name).join('、')}</div>
        </div>`).join('')}
      </div>
    ...
    <div class="main">
      ...
      <div class="m-section">
        ...
        <div class="timeline-item">
          ...
          ${p.subProjects ? p.subProjects.slice(0, 2).map((s, j) => `
          <div class="project-card">
            <div class="card-title">${j + 1}. ${s.title}${s.subtitle ? `<span class="card-subtitle">(${s.subtitle})</span>` : ''}</div>
            <div class="card-desc">${s.description}</div>
            ${s.techStack ? `<div class="card-tech"><strong>技术栈：</strong>${s.techStack.join('、')}</div>` : ''}
          </div>`).join('') : ''}
  ...
  console.log('生成 PDF...');
  const outputPath = path.join(__dirname, '..', 'public', '大鱼_简历.pdf');
