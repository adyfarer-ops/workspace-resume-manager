...
SET content = '<article class="prose prose-stone max-w-none">
  ...
  <div class="my-8">
    <img src="/images/article/openclaw-dashboard.png" alt="OpenClaw Dashboard" class="w-full max-w-3xl mx-auto rounded-lg shadow-md border border-stone-200" />
    <p class="text-center text-sm text-stone-500 mt-2">▲ OpenClaw 管理界面，显示 Connected 状态表示连接成功</p>
  </div>
  
  <p class="leading-relaxed mb-6">然后访问 http://你的公网IP:18789，看到页面显示 "Connected" 就对了。</p>
  ...
  <div class="my-8">
    <img src="/images/article/openclaw-config.png" alt="OpenClaw 配置" class="w-full max-w-2xl mx-auto rounded-lg shadow-md border border-stone-200" />
    <p class="text-center text-sm text-stone-500 mt-2">▲ openclaw.json 中的 wecom-app 插件配置</p>
  </div>
  ...
  <p class="leading-relaxed mb-4">上传 Logo，填应用名称，设置可见范围。</p>
  ...
  <p class="leading-relaxed mb-4">进入应用详情 → 接收消息 → 设置 API 接收：</p>
  
  <div class="my-8">
    <img src="/images/article/wecom-app-config.png" alt="企业微信应用配置" class="w-full max-w-3xl mx-auto rounded-lg shadow-md border border-stone-200" />
    <p class="text-center text-sm text-stone-500 mt-2">▲ 企业微信应用管理页面，显示"已启用API接收"</p>
  </div>
  
  <ul class="list-disc pl-6 mb-6 space-y-2">
    <li>Token 和 EncodingAESKey：点"随机获取"</li>
    <li>URL：填 http://&lt;公网IP&gt;:18789/wecom/app</li>
  </ul>
  
  <p class="leading-relaxed mb-4"><strong>注意</strong>：这时候先别点保存，因为 OpenClaw 那边还没配好，保存会报错。</p>
  
  <h3 class="text-lg font-semibold mt-6 mb-3">3.3 获取企业 ID</h3>
  
  <p class="leading-relaxed mb-4">去"我的企业"页面，复制 CorpID。</p>
  
  <div class="my-8">
    <img src="/images/article/wecom-corpid.png" alt="企业微信企业ID" class="w-full max-w-3xl mx-auto rounded-lg shadow-md border border-stone-200" />
    <p class="text-center text-sm text-stone-500 mt-2">▲ 企业微信"我的企业"页面，底部显示企业ID</p>
  </div>
  ...
  <pre class="bg-stone-100 p-4 rounded-lg mb-6 overflow-x-auto text-sm"><code>{
  "wecom-app": {
    "enabled": true,
    "webhookPath": "/wecom/app",
    "token": "从企业微信后台获取的 Token",
    "encodingAESKey": "从企业微信后台获取的 AESKey",
    "corpId": "你的企业 ID",
  ...
  <p class="leading-relaxed mb-4">保存，重启 OpenClaw。确认没报错后，回到企业微信后台，点保存 API 接收 URL。</p>
  
  <p class="leading-relaxed mb-6">如果显示"请求成功"，说明消息通道已经打通了！</p>
  
  <h2 class="text-xl font-bold mt-8 mb-4">第五步：绑定微信端</h2>
  
  <p class="leading-relaxed mb-4">在企业微信后台找到"微信插件"功能页，上传应用 Logo，扫二维码完成绑定。</p>
  
  <p class="leading-relaxed mb-4">然后打开微信 → 通讯录 → 我的企业，就能看到 OpenClaw 应用了。</p>
  
  <p class="leading-relaxed mb-6">点进去，开始聊天！🎉</p>
  ...
  <p class="leading-relaxed mb-4">实现方式是用 Claude Code 的 Skill Creator 创建了一个 skill，识别到 mp.weixin.qq.com 开头的链接就触发，自动抓取文章内容、生成总结，调用飞书 API 存表。</p>
  
  <p class="leading-relaxed mb-6">测试了一下，效果还不错。文章链接发过去，几秒钟后飞书表里就有记录了，包括链接、标题、总结、核心观点、创建时间。</p>
